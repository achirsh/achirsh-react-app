    ...
    dingwei() {
        ...
        geolocation.getCurrentPosition((status: any, result: any) => {
            if (status === "complete") {
                ...
                Toast.show({ content: JSON.stringify(result) })
                ...
                geocoder.getAddress(result.position, (status: any, result: any) => {
                    if (status === "complete" && result.regeocode) {
                        ...
                        this.setState({ visible1: false }, () => {
                            const data: any = adcodes.filter(x => address.includes(x.name))
                            if (data.length) {
                                this.clearAllLayout()
                                this.selectProvince = data[0]

                                const info = data[0].city.citys.filter((x: any) => x.name === searchText)

                                if (info.length) {
                                    // 定位到三级区域
                                    this.map.setZoomAndCenter(zoomsData[2], info[0].center)
                                    info[0].have.forEach((n: any) => {
                                        info[0][n].forEach((n1: any) => {
